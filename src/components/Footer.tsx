...
export default function Footer() {
  ...
  // Parallax mapping: when scrolling down, text moves right (from negative offset to positive)
  const xTransform = useTransform(scrollYProgress, [0, 1], ["-20%", "5%"]);
  ...
  return (
    <footer id="footer" ref={ref} className="bg-[var(--alt-bg)] text-[var(--foreground)] w-full border-t border-[var(--border-subtle)] transition-colors overflow-hidden">
      ...
      <div className="w-full border-b border-[var(--border-subtle)] bg-[var(--alt-bg)] py-12 md:py-16 relative flex items-center select-none overflow-hidden group">
        <motion.div 
            style={{ x: xTransform }}
            className="flex whitespace-nowrap"
        >
           <h2 className="text-[4rem] sm:text-[6rem] md:text-[8rem] lg:text-[12rem] xl:text-[15rem] leading-none font-black uppercase tracking-tight pr-16 bg-gradient-to-br from-[var(--foreground)] via-[var(--foreground)]/50 to-[var(--background)] bg-clip-text text-transparent opacity-10">
               TRADEPIVOT — TRADEPIVOT — TRADEPIVOT — TRADEPIVOT
           </h2>
        </motion.div>
      </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="relative h-16 w-64 -ml-4">
              <Image 
                ...
                alt="Tradepivot" 
                fill
                className="object-contain dark:invert-0 light:invert transition-all"
              />
            </div>
