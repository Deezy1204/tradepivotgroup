...
export default function Hero() {
  ...
  return (
    <section ref={targetRef} className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[var(--background)]">
      {/* Background with Metallic/Steel Overlay */}
      <motion.div 
        style={{ scale }}
        className="absolute inset-0 z-0 bg-[url('https://images.unsplash.com/photo-1517581177682-a085bb7ffb15?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center transition-all duration-700"
      >
        <div className="absolute inset-0 bg-[var(--background)]/60 dark:bg-black/50 z-[5]" />
        <div className="absolute inset-0 bg-gradient-to-b from-[var(--background)]/95 via-[var(--background)]/60 to-[var(--background)] z-10" />
      </motion.div>
